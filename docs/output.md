...
The undetermined components do not continue through the rest of the pipeline, so
their PXL file is delivered directly at the `pixelator/` root, next to the final
(layout) PXL files. This is only done when `pna_sample_calling_save_undetermined = true`
(the default). The intermediate per-sample dehashed PXL files continue downstream
and are only copied to the `sample_calling` folder when `save_pna_sample_calling_pixelfile = true`.
...
- `pixelator`
  ...
  - `sample_calling`
    - `<sample-id>.dehashed.pxl`: The pixel file containing the data associated with each samples. Only saved when `save_pna_sample_calling_pixelfile = true`.
    - `<sample-id>.meta.json`: Command invocation metadata.
...
generated by the pipeline (with the most "complete" information) and an interactive HTML report summarizing the data for all samples.
...
The `<pool-id>_undetermined.dehashed.pxl` file is also delivered here by default, since these
components are not processed any further by the pipeline.
...
> [!WARNING]
> If the `layout` stage is skipped (using `--skip_layout`), use `--save_pna_<last running step>` to output the latest PXL files.
...
- `pixelator/`
  ...
  - `<pool-id>_undetermined.dehashed.pxl`
